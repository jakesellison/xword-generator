...
dict_init <- function(word_len_list, level = 85, data_buzzwords){
  ...
  dict <- bind_rows(dict,pb_words) %>% 
    select(word) %>%
    rbind(data.frame(word = data_buzzwords)) %>% 
    distinct(word)
  dict <- dict %>% 
...
rm_word <- function(depth){
  ...
  return(ifelse(depth-1 == 0, 1, depth-1))
}
...
main <- function(seed, data_buzzwords){
  #browser()
  down <- c()
  while(length(down) != length(stack_dict)){ #depth is the index progress through stack_dict
    ...
    while(rlang::is_empty(stack_dict[[depth]])){
      if (depth == 0) {
        ...
        if(length(affected_words(depth)) == 1){
          #if there is only a single dependency in the previous col, go to the earliest word in the puzzle that is affected by current failure
          matched_pos <- match(affected_words(depth), word_pos)
        } else if(length(affected_words(depth)) > 1){
          #go to position of the most recently added word in the puzzle that is affected by current failure
          matched_pos <- match(max(affected_words(depth)), word_pos) 
        }
        ...
        down <- down[1:depth]
        depth <- ifelse(depth == 0, depth, depth + 1)
    ...
    if(rlang::is_empty(stack_dict[[depth+1]])){browser()}
    
    data_words <- stack_dict[[depth]][data_buzzwords %in% stack_dict[[depth]]]
    if(length(data_words) > 1){
      candidate <- data_words[1]
      stack_dict[[depth]] <- stack_dict[[depth]][stack_dict[[depth]] != candidate] # remove the chosen word from dict
    } else {
      candidate <- stack_dict[[depth]][1]
      stack_dict[[depth]] <- stack_dict[[depth]][-1]
    }
    if(is.na(candidate)){browser()}
    down[depth] <- candidate
    depth <- write_word(candidate, depth, type = 'str')
    runlog <<- c(runlog, depth)
    if(candidate == 'charter' & depth == 4 & down[1] == 'woo'){browser()}
  }
  return(puzz)
}
...
#stack_dict <<- dict_init(word_lens, level = 85, data_buzzwords)
