...
puzzle <- puzzle_init()
depth <- 3
avail_words <- as.vector(stack_dict)
for(i in 1:(depth-1)){
  puzzle <- write_word(clues[i],puzzle,i)
}
clue <- 'hmph'
...
profvis({
  ...
  for(i in 1:nrow(puzzle)){
    regex_clue <- regex(paste(unlist(puzzle[i,]), collapse=''))
    for(j in 1:length(avail_words)){
      match <- append(match,stringi::stri_detect_regex(avail_words[j],regex_clue))
      # if(stringi::stri_detect_regex(avail_words[j],regex_clue) == TRUE){
      #   match_list[i] <- TRUE
      #   break
      # }
    }
  }
  pass <- ifelse(length(match_list[is.na(match_list)]) == 0, TRUE, FALSE)
})
