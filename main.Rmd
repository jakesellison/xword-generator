...
```{r}
subset_dict <- function(across, req_char, dict){
  #get all valid words
  valid <- c() ; invalid <- c()
  for(i in 1:length(across)){
    valid <- append(valid, list(names(frag_hash[[paste(across[i],req_char[i],sep='')]]$valid)))
  }
  for (i in 1:length(valid)){
    dict <- dict[substr(dict,i,i) %in% valid[[i]]]
  }
  return(dict)
}
...
main <- function(){
  ...
  while(depth <= dim){
    ...
    stack_dict[[depth]] <- subset_dict(across,req_char,stack_dict[[depth]]) #returns a list of all valid words
    ...
    for(i in depth:1){
      if(rlang::is_empty(stack_dict[[i]])){ #out of dict words at this level
        ...
        if(length(down) != depth-1){browser()}
        break
      }
    }
