...
find_word <- function(clue_matrix,stack_dict,frag_array,dim,clues){
  for(i in 1:length(stack_dict)){
    ...
    if(!FALSE %in% try){
      return(c(stack_dict[i],list(stack_dict[(i+1):length(stack_dict)]),TRUE))
    }
  }
...
main <- function(seed, dim){
  ...
  frag_array <- array(rep(0,27^dim),rep(27,dim))
  frag_array <- populate_frag_array(frags, frag_array, dim)
  ...
  while(length(clues) != dim){
    ...
    if(search_tpl[[3]] == FALSE & depth == 2){
      return(FALSE)
    } else if (search_tpl[[3]] == FALSE) {
      ...
      clue_matrix <- cbind(clue_matrix, search_tpl[[1]])
      clues <- append(clues, paste(search_tpl[[1]],collapse=''))
    }
  }
  return(clue_matrix)
}
